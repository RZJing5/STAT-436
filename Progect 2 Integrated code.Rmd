...
```{r}
#read data set
dataset <- read_csv("https://raw.githubusercontent.com/Star732/.csvFile/main/world_population.csv")
names(dataset)[3] <- "region"
```
```{r}
# change colnames so that they could be used conveniently
colnames(dataset) <- c("Rank", "CCA3", "region", "Capital", "Continent", "year_2022",
...
#
plot_world_heatmap <- function(df_map) {
  ggplot() +
    geom_map(data = df_map, map = world_map, aes(fill = year_2022,
                                                  map_id = region),
...
ui <- fluidPage(
  ...
  selectInput("countries", "Country", choices = df1$region, multiple = TRUE),
  checkboxGroupInput("years", "Year", choices = unique(df1$Year)),
  plotOutput("scatter"),
  plotOutput("area")
)
...
server <- function(input, output, session) {
  ...
  output$scatter <- renderPlot({
    df1 %>%
      ...
      ggplot() +
      geom_point(aes(Year, region, size = log10(Population), alpha = Growth_rate), col = "blue") +
  ...
  output$area <- renderPlot({
    df1 %>%
      ...
      ggplot() +
      geom_area(aes(Year, World_Population_Percentage, group = CCA3, fill = CCA3), alpha = 0.8, col = "white") +
      #ylim(0, 100)+
